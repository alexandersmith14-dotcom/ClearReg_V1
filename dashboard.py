...
/* Text-slide hover, stolen from the "5 Trendy Animations" reference video:
   the current label slides up and out on hover while a duplicate (in
   --accent, via ::after so it never needs a second real DOM node) slides up
   into its place from below. `data-label` on .lbl is kept in sync with its
   textContent in JS (see initQuickStart) since the button's own text
   changes between "Next" and "Got it". Scoped to real mouse hover only --
   `pointer:fine` keeps this off touch devices entirely, so tapping never
   leaves it stuck mid-slide with no hover-out event to reverse it; the
   plain button (no slide) is what mobile gets, by design, not a fallback
   bug. Desktop first, mobile intentionally simpler, per the reference
   video's own point 5 about needing a non-hover backup. */
.btn-slide{position:relative;overflow:hidden}
.btn-slide .lbl{position:relative;display:block}
@media (hover:hover) and (pointer:fine){
  .btn-slide .lbl{transition:transform .28s cubic-bezier(.65,0,.35,1)}
  .btn-slide .lbl::after{content:attr(data-label);position:absolute;left:0;top:100%;
    color:var(--accent)}
  .btn-slide:hover .lbl{transform:translateY(-100%)}
}
...
function initQuickStart() {
  ...
  const qsLbl = qsBtn && qsBtn.querySelector('.lbl');
  if (!qs || !qsText) return;
  let step = 0;
  ...
  function showStep(i) {
    ...
    const label = hasMore ? 'Next' : 'Got it';
    qsLbl.textContent = label;
    qsLbl.dataset.label = label;
    qs.hidden = false;
    place(target);
  }
  function finish() {
...
<div id="quickstart" class="quickstart" role="dialog" aria-label="Quick start" hidden>
  ...
  <div class="qs-foot"><button id="qsDismiss" type="button" class="btn-slide">
    <span class="lbl" data-label="Got it">Got it</span></button></div>
</div>
